...
회원가입없음. 페이지·진행/결과 화면은 로그인 없이 볼 수 있지만(부스 스크린에 그대로 띄우기 위함), 
**제출(=Codex 호출·과금)은 로그인해야만 가능**. 제출 폼/버튼은 로그인한 계정에게만 보이고, 
익명의 제출 POST는 로그인으로 리다이렉트된다(랜덤 유입의 할당량 소모 차단).
...
- `config/scoring.yaml` — 가중치·임계값·감점·게이트 캡·등급 컷·컨테이너 제한·
  Codex 플래그·외부 도구·오케스트레이터 동시성/재시도.
