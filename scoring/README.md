...
> **SSRF에는 정적 검사가 없다.** 예전의 `ssrf_sink`(외부 요청 함수의 첫 인자가 리터럴이
> 아니면 감점)는 **제거했다.** 데이터 흐름 분석 없이는 `urlopen(url)` 한 줄만 보고 가드된 구현과
> 취약한 구현을 구분할 수 없어, 사설 IP를 차단하는 앱도 감점당했다. 반대로 가드가 있다고
> 봐주면 실제 우회(공격자 공개 호스트의 302 리다이렉트 → 내부 주소)를 놓친다. 어느 쪽으로
> 조정해도 틀리는 검사라, SSRF는 **동적 `ssrf` 프로브로만** 판정한다 — 콜백이 실제로 도착한
> 경우에만 감점하므로 오탐이 원천적으로 불가능하고, 판정 못 하면 skip한다.
