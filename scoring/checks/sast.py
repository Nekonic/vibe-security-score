...
def check_semgrep(check, sctx, cfg):
    config = sctx.config
    tcfg = _tool_cfg(config, "semgrep")
    bin_path, skip = _gate(config, "semgrep")
    if bin_path is None:
        return _skipped(check.id, check.label, "semgrep", skip or _SKIP_REASON)

    ruleset = tcfg.get("ruleset", "p/security-audit")
    cmd = _stub_command(bin_path) + ["--config", ruleset, "--json", "--quiet", sctx.app_dir]
    ...
    return result(check, cfg, score=0.0 if reasons else 100.0, passed=not reasons,
                  reasons=reasons, evidence=evidence, tool="semgrep")
...
CHECKS = [
    Check("semgrep", "정적 룰셋(semgrep)", "static", check_semgrep,
