...
from scoring.static import tools as tools_mod
from scoring.static.dependency_checks import parse_requirements
...
def test_pypi_disabled_by_default_no_network(base_config):
    ...
    r = tools_mod.check_pypi_existence(reqs, base_config)
    assert r.skipped is True
    assert r.tool == "pypi"
    assert r.weight == 0.0
